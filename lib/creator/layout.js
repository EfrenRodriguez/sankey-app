...
export function computeButterfly({ leftNodes, rightNodes, links, total }) {
  ...
  // ── Pre-calculate total ribbon thickness per node (center-anchored fans) ────
  // totalLThick[id] = sum of tL leaving that left node
  // totalRThick[id] = sum of tR arriving at that right node
  // totalMThick is not needed: it always equals midSeg.h, so mCursor = s.y.
  const totalLThick = {};
  const totalRThick = {};

  leftNodes.forEach(leftNode => {
    totalLThick[leftNode.id] = links
      .filter(l => l.leftId === leftNode.id && l.value > 0)
      .reduce((sum, link) => sum + Math.max(1.5, (link.value / total) * (AVAIL * 0.68)), 0);
  });
  links.forEach(link => {
    if (link.value <= 0) return;
    const tR = Math.max(1.5, (link.value / total) * (AVAIL * 0.72));
    totalRThick[link.rightId] = (totalRThick[link.rightId] || 0) + tR;
  });

  // ── Ribbon cursors — centered on each node's midpoint ────────────────────
  // lCursor / rCursor start at node.cy − totalThick/2 so the bundle fans
  // symmetrically above and below the node centre.
  // mCursor starts at s.y because totalMThick ≡ midSeg.h (s.y = s.cy − s.h/2).
  const lCursor = {};
  typeNodes  .forEach(n => { lCursor[n.id] = n.cy - (totalLThick[n.id] || 0) / 2; });
  const mCursor = {};
  midSegments.forEach(s => { mCursor[s.id] = s.y; });
  const rCursor = {};
  clubNodes  .forEach(n => { rCursor[n.id] = n.cy - (totalRThick[n.id] || 0) / 2; });
  ...
  leftNodes.forEach(leftNode => {
    // Sort descending so the dominant ribbon is placed first (top of bundle).
    const nodeLinks = links
      .filter(l => l.leftId === leftNode.id)
